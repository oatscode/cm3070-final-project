...
public class Food : MonoBehaviour {
    ...
    public void OnEatenTrigger(UIManager uiManager, GameController gameController) {
        uiManager.AddScore(basePoints); // add the base points to the score
        ...
        if (powerUpType == PowerUpType.Rotten) {
            gameController.ActivateRottenPower();
        } else if (powerUpType != PowerUpType.None) {
            gameController.StorePowerUp(powerUpType);
        }
